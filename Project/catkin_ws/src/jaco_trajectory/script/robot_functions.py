...
def robotHessian(DH_mat):
    # Symbolize state variables
    q1, q2, q3, q4, q5, q6 = symbols('q1 q2 q3 q4 q5 q6')
    q = Matrix([[q1],[q2],[q3],[q4],[q5],[q6]])
    DH_mat_lin = hessian(DH_mat,q)
    return DH_mat_lin

def robotHessian_type(DH_mat):

    Jplus = DH_mat.transpose()               # Calculate transpose
    Jplus1=Jplus*DH_mat
    

    return Jplus1

def evalHessian(J,state):
    q1, q2, q3, q4, q5, q6 = symbols('q1 q2 q3 q4 q5 q6')
    J = J.subs(q1,state[0])
    J = J.subs(q2,state[1])
    J = J.subs(q3,state[2])
...
def evalCond(J,state,lamba1,delta_row):
    q1, q2, q3, q4, q5, q6 = symbols('q1 q2 q3 q4 q5 q6')
    lamba=symbols('lamba')
    ...
    J = J.subs(lamba,lamba1)

    J = J.subs(delta_v1,delta_row[0])
    J = J.subs(delta_v2,delta_row[1])
    J = J.subs(delta_v3,delta_row[2])
    J = J.subs(delta_v4,delta_row[3])
    J = J.subs(delta_v5,delta_row[4])
    J = J.subs(delta_v6,delta_row[5])

    return J.evalf()   

def evalCond1(J,state,lamba1):
    q1, q2, q3, q4, q5, q6 = symbols('q1 q2 q3 q4 q5 q6')
    lamba=symbols('lamba')
    J = J.subs(q1,state[0])
    J = J.subs(q2,state[1])
...
#gaussian newton's method without equality constraints
def inverseKinematics_type(J,step,delta_X):
    J = Sym2NumArray(J)             # Convert to numpy.array
    J = numpy.array(J).astype(numpy.float64)    # Convert to numpy.matrix
    Jplus = J.transpose()               # Calculate transpose
    Jplus1=numpy.dot(Jplus,J)
    Jplus2=numpy.linalg.pinv(Jplus1)
    Jplus3=numpy.dot(Jplus2,Jplus)
    # Jplus = numpy.linalg.pinv(J)                # Calculate pseudoinverse
    # print "Jplus3:"
    # print Jplus3
    # print "delta_X:"
    # print delta_X
    delta_q = Jplus3 * delta_X           # Calculate change in joint space
    return delta_q


#newton's method without equality constraints
def inverseKinematics_newton(J,step,delta_X):
    J = Sym2NumArray(J)             # Convert to numpy.array
    J = numpy.array(J).astype(numpy.float64)    # Convert to numpy.matrix
    Jplus = delta_X.transpose()               # Calculate transpose
    
    Jplus1 = numpy.linalg.pinv(J)                # Calculate pseudoinverse
    # print "Jplus3:"
    # print Jplus3
    # print "delta_X:"
    # print delta_X
    delta_q = Jplus1 * Jplus           # Calculate change in joint space
    return delta_q

#newton's method with equality constraints
def inverseKinematics_newton_con(J,step,delta_X):
    J = Sym2NumArray(J)             # Convert to numpy.array
    J = numpy.array(J).astype(numpy.float64)    # Convert to numpy.matrix
    
    delta_X = Sym2NumArray(delta_X)             # Convert to numpy.array
    delta_X = numpy.array(delta_X).astype(numpy.float64) 

    delta_q=numpy.linalg.pinv(J)*delta_X
