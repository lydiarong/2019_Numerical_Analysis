...
if __name__ == '__main__':
  try:
	...
	for i in range(1,numPts):
	  ...
	  path_Z = Matrix([[zPath[i]]])#,[yPath[i]],[zPath[i]]
	  print(i)
	  # print "init_q:"
	  # print init_q
	  # print init_X
	  ...
	  HJ_x = robotJacobian((path_X-DynMatrix[0])**2)
	  H_x = robotHessian((path_X-DynMatrix[0])**2)
	  print H_x.shape ,HJ_x.shape

	  itertion = 0
	  while (Abs(delta_X.norm()) > error): #or (Abs(delta_Y.norm()) > error) or (Abs(delta_Z.norm()) > error):
